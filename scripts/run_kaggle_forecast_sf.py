#!/usr/bin/env python3
"""Semi-finals WC 2026 forecast using pre-trained models (post-QF results)."""
...
SF_TOURNAMENT = (
    PROJECT_ROOT / "config" / "tournaments" / "world_cup_2026_semifinals.yaml"
)


def main() -> int:
    parser = argparse.ArgumentParser(
        description="WC 2026 semi-finals forecast (knockout from SF, post-QF results)"
    )
    parser.add_argument(
        "--profile",
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Default: models_dir/wc2026_forecast_sf.json",
    )
    ...
    forecast_path = args.output or models_dir / "wc2026_forecast_sf.json"
    report_path = models_dir / "forecast_sf_report.json"
    ...
    print(f"\nSF forecast complete: {forecast_path}")
